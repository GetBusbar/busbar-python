...
def sync_detailed(
    *,
    ...
    """ROTATE the busbar key-signing key. Rotation is REVOKE-ALL by design: a new signing key means every
    token minted under the OLD key stops verifying, so every outstanding key must be re-minted. 1.5.0 is
    single-key, so this reports the intent + current kid; the actual swap is an operator action (replace
